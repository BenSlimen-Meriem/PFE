...
import { AffaireService, EntityArrayResponseType } from '../service/affaire.service';
...
export class AffaireComponent implements OnInit {
  ...
  affaires = signal<IAffaire[]>([]);
  isLoading = false;

  sortState = sortStateSignal({});

  public readonly router = inject(Router);
  protected readonly affaireService = inject(AffaireService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IAffaire): number => this.affaireService.getAffaireIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.affaires().length === 0) {
            ...
            this.affaires.set(this.refineData(this.affaires()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.affaires.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IAffaire[]): IAffaire[] {
