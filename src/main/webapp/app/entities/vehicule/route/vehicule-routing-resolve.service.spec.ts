...
import vehiculeResolve from './vehicule-routing-resolve.service';

describe('Vehicule routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: VehiculeService;
  let resultVehicule: IVehicule | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IVehicule returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        vehiculeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultVehicule).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IVehicule>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
